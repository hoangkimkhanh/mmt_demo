...
def get_log_path(error):
    ...
    if error:
        ...
        log_path = os.path.join(
            settings.LOGGER_ERROR_PATH, '{:%Y-%m-%d}-Searching-server-error.log'.format(datetime.now()))
    else:
        set_logger_dir(settings.LOGGER_INFO_PATH)
        log_path = os.path.join(
            settings.LOGGER_INFO_PATH, '{:%Y-%m-%d}-Searching-server-info.log'.format(datetime.now()))
